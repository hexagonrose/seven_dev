...
if __name__ == '__main__':
    ...
    cif_paths = glob(os.path.join(log_mat_path, f'{material}*.cif'))

    for file_path in cif_paths:
        mat_full_name = os.path.basename(file_path).split('.')[0]
        log_file = os.path.join(log_mat_path, f'{mat_full_name}.txt')
        ...
        write_log(log_file, f'Average time : {sum(time_list) / len(time_list)} s.')
